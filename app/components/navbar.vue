  ...
  <nav class="bg-white shadow-md px-6 py-4 flex justify-between items-center relative">
    ...
    >
      <div v-for="(item, index) in menuItems" :key="item.to" class="w-full">
        <NuxtLink 
          :to="item.to" 
          class="block w-full py-2 text-gray-700 hover:text-blue-600 transition-colors"
          @click="item.children ? toggleMobileDropdown(index) : mobileOpen = false"
        >
          {{ item.label }}
        </NuxtLink>
        ...
        <!-- Mobile Dropdown -->
        <ul 
          v-if="item.children && mobileDropdown === index" 
          class="pl-4 mt-1 border-l border-gray-300"
        >
          <li v-for="child in item.children" :key="child.to">
            <NuxtLink 
              :to="child.to" 
